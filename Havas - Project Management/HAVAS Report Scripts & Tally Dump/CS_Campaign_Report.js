...
define([], function () {
  ...
  function exportReport() {
    try {
      ...
      let csvContent ='Creation Date,Project Code,Project Name,Location,Department,Customer,Start Date , End Date , Closure Date, Billing Budget,SO Value,SB Value,JV Sales,CN Value,Net Sales,Cost Budget,PO Value,PB Value,Expense Report,Total PO,Total PB,JV Purchase,DN Value,Net Expense,Net Profit\n';
      ...
      for (let i = 1; i <= rowCount; i++) {
        ...
        const projectName = nlapiGetLineItemValue(sublistId, 'custpage_project_name', i);
        ...
        const startDate = nlapiGetLineItemValue(sublistId, 'custpage_start_date', i);
        const endDate = nlapiGetLineItemValue(sublistId, 'custpage_end_date', i);
        const closureDate = nlapiGetLineItemValue(sublistId, 'custpage_closure_date', i);
        ...
        const row = [
          ...
          formatCSVCell(code),
          formatCSVCell(projectName),
          formatCSVCell(location),
          formatCSVCell(department),
          formatCSVCell(customer),
          formatCSVCell(startDate),
          formatCSVCell(endDate),
          formatCSVCell(closureDate),
