...
define(["N/ui/serverWidget", "N/search"], (serverWidget, search) => {
  ...
  let generateRequestForm = () => {
    ...
    form.addField({ id: "custpage_vendors", label: "Vendor", type: serverWidget.FieldType.MULTISELECT, source: "vendor" });
  ...
  let generateOpeningBalances = (fromDate, uptoDate, selectedVendors, selectedLocations) => {
    ...
    form.addButton({ id: "Export Excel", label: "custbody_export_excel", functionName: "exportToExcel()" });
  ...
  let getAllVendors = () => {
    let vendorSearchObj = search.create({
      type: "vendor",
      filters: [],
      columns: [search.createColumn({ name: "internalid", label: "Internal ID" }), search.createColumn({ name: "entityid", label: "Name" })],
    });
  ...
  let getAllVendorOpeningBalance = (date, selectedLocations) => {
    let filters = [
      [[["type", "anyof", "ExpRept", "VendCred", "VendPymt", "Custom108", "Journal"], "AND", [["accounttype", "anyof", "AcctPay"], "OR", ["account", "anyof", "2356"]]], "OR", [["type", "anyof", "VendBill", "VPrep"], "AND", ["mainline", "is", "T"]]],
      "AND",
