...
define(["N/ui/serverWidget", "N/search"], (serverWidget, search) => {
    ...
    let generateOpeningBalances = (fromDate, uptoDate, selectedCustomers, selectedLocations, selectedDepartment) => {
        ...
        allDebit.forEach(projectBalance => {
            ...
            if (_logValidation(customerID)) {
                let projectID = getProjectCode(projectBalance.getValue({ name: "entityid", join: "job", summary: "GROUP" }));
        ...
        for (let count = 0; count < customerLines.length; count++) {
            ...
            for (let projectName in projectLines) {
                ...
                let drillURL = 'https://9370186.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1391&deploy=1&projectCode=' + getProjectCode(projectName) + '&startDate=' + fromDate + '&endDate=' + uptoDate;
    ...
    let getDebit = (fromDate, uptoDate, selectedLocations, selectedDepartment) => {
        ...
        let customerSearchObj = search.create({
            ...
            columns: [
                ...
                search.createColumn({ name: "entityid", join: "job", summary: "GROUP" }),
    ...
    let getDelgatesSoCn = (fromDate, uptoDate, selectedLocations, selectedDepartment) => {
        let filter = [["job.internalidnumber", "isempty", ""], "AND", ["type", "anyof", "SalesOrd", "CustInvc", "CustCred"], "AND", ["cseg_ags_projectcod", "noneof", "@NONE@"], "AND", ["mainline", "is", "F"], "AND", ["taxline", "is", "F"], "AND", ["trandate", "within", fromDate, uptoDate]];
        ...
        var delegatesSearchObj = search.create({
            ...
            columns:
                [
                    search.createColumn({ name: "formulatext", summary: "GROUP", formula: "CASE      WHEN {cseg_ags_projectcod} IS NOT NULL THEN          CASE              WHEN INSTR({cseg_ags_projectcod}, ' ') > 0 THEN SUBSTR({cseg_ags_projectcod}, 1, INSTR({cseg_ags_projectcod}, ' ') - 1)              ELSE {cseg_ags_projectcod}          END      ELSE NULL  END", label: "Project Code" }),
