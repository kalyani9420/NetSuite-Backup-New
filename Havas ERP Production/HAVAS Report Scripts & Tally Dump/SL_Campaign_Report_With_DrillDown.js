...
define(["N/ui/serverWidget", "N/search", "N/url"], (serverWidget, search, url) => {
    ...
    const THIS_SUITELET_INTERNAL = { scriptId: "customscript1399", deploymentId: "customdeploy1" };
    ...
    const idsFromMulti = (val) => (val && val !== SEP) ? val.split(SEP).filter(Boolean) : [];

    const joinNamesByIds = (type, ids) => {
        const arr = Array.isArray(ids) ? ids : idsFromMulti(ids);
        if (!arr.length) return "All";
        const s = search.create({
            type,
            filters: [["internalid", "anyof", arr]],
            columns: [
        ...
        const rows = getAllResult(s);
        const names = rows.map(r =>
            r.getValue({ name: "entityid" }) || r.getValue({ name: "name" }) || r.getText({ name: "name" })
        ).filter(Boolean);
        return names.length ? names.join(", ") : arr.join(", ");
    };


    const nameById = (type, id) => {
        if (!isValid(id)) return "All";
        try {
            const o = search.lookupFields({ type, id, columns: ["name"] });
    ...
    const TXN_TYPE_TO_SCRIPTID = { "Sales Order": "salesorder", "Invoice": "invoice", "Credit Memo": "creditmemo", "Purchase Order": "purchaseorder", "Vendor Invoice": "vendorbill", "Expense Report": "expensereport", "Bill Credit": "vendorcredit", "Journal": "journalentry" };
    ...
    const onRequest = (ctx) => {
        try {
            ...
            const form = generateCampaignReport(fromDate, uptoDate, selectedCustomers, selectedProjects, selectedLocations, selectedDepartment);
    ...
    const generateRequestForm = () => {
        const form = serverWidget.createForm({ title: "Campaign Report" });
    ...
    const generateCampaignReport = (fromDate, uptoDate, selectedCustomers, selectedProjects, selectedLocations, selectedDepartment) => {
        const form = serverWidget.createForm({ title: `Campaign Report [ ${toStr(fromDate)} to ${toStr(uptoDate)} ]` });
        form.clientScriptModulePath = "SuiteScripts/CS_Campaign_Report.js";
        ...
        const fg = form.addFieldGroup({ id: "custpage_fg_filters", label: "Filters" });

        const fFrom = form.addField({ id: "custpage_from_date", label: "Start / From Date", type: serverWidget.FieldType.DATE, container: "custpage_fg_filters" });
        fFrom.defaultValue = toStr(fromDate, "");

        const fTo = form.addField({ id: "custpage_upto_date", label: "End / Upto Date", type: serverWidget.FieldType.DATE, container: "custpage_fg_filters" });
        fTo.defaultValue = toStr(uptoDate, "");

        const fCust = form.addField({ id: "custpage_customer", label: "Customer", type: serverWidget.FieldType.MULTISELECT, source: "customer", container: "custpage_fg_filters" });

        fCust.defaultValue = selectedCustomers || "";

        const fProj = form.addField({ id: "custpage_project", label: "Project", type: serverWidget.FieldType.MULTISELECT, source: "job", container: "custpage_fg_filters" });
        fProj.defaultValue = selectedProjects || "";

        const fLoc = form.addField({ id: "custpage_locations", label: "Location", type: serverWidget.FieldType.SELECT, source: "location", container: "custpage_fg_filters" });
        fLoc.defaultValue = selectedLocations || "";

        const fDept = form.addField({ id: "custpage_departments", label: "Department", type: serverWidget.FieldType.SELECT, source: "department", container: "custpage_fg_filters" });
        fDept.defaultValue = selectedDepartment || "";
        ...
        log.debug('projectTransaction', projectTransaction)
        ...
        projectTransaction.forEach(project => {
            ...
            if ((!selectedProjectsArray.length || selectedProjectsArray.includes(String(id))) && isValid(id)) {
                ...
                const startDate = project.values["GROUP(formuladate)_1"];
                const endDate = project.values["GROUP(formuladate)_2"];
                const closureDate = project.values["GROUP(formuladate)_3"];
                ...
                const projectName = project.values["GROUP(formulatext)_6"];
                ...
                if (soValue == 0 && sbValue == 0 && jvSale == 0 && cnValue == 0 && poValue == 0 && pbValue == 0 && expReport == 0 && jvPurchase == 0 && dnValue == 0) { }
                    ...
                    projectObject[id] = { id, code, creationDate, location, department, customer: cleanDisplayName(customer, true), projectName, startDate, endDate, closureDate, billingBudget, costBudget };
        ...
        log.debug('delegatesSoSbCn', delegatesSoSbCn)
            ...
            if (Object.prototype.hasOwnProperty.call(projectIdObject, delgCode)) {
                ...
                if (delgProject.length) {
                    ...
                    if ((!selectedProjectsArray.length || selectedProjectsArray.includes(String(id))) && isValid(id)) {
                        ...
                        projectObject[id] = { id, code: delgCode, creationDate: p.values["formuladate"], location: p.values["formulatext"], department: p.values["formulatext_1"], customer: cleanDisplayName(p.values["formulatext_2"], true), projectName: p.values["formulatext_3"], billingBudget: p.values["formulatext_4"], costBudget: p.values["formulatext_5"], startDate: p.values["formuladate_1"], endDate: p.values["formuladate_2"], closureDate: p.values["formuladate_3"], };
        ...
        // log.debug('Project object', projectObject);
        ...
        sublist.addField({ id: "custpage_project_name", type: serverWidget.FieldType.TEXT, label: "Project Name" });
        ...
        sublist.addField({ id: "custpage_start_date", type: serverWidget.FieldType.TEXT, label: "Start Date" })
        sublist.addField({ id: "custpage_end_date", type: serverWidget.FieldType.TEXT, label: "End Date" })
        sublist.addField({ id: "custpage_closure_date", type: serverWidget.FieldType.TEXT, label: "Closure Date" })
        ...
        sublist.addField({ id: "custpage_actual_margin", type: serverWidget.FieldType.TEXT, label: "Net Profit" });
        ...
        const projectLines = Object.entries(projectObject).sort((a, b) => {
            const [aDay, aMonth, aYear] = a[1].creationDate.split('/').map(Number);
            const [bDay, bMonth, bYear] = b[1].creationDate.split('/').map(Number);
            const aKey = aYear * 10000 + aMonth * 100 + aDay;
            const bKey = bYear * 10000 + bMonth * 100 + bDay;
            return bKey - aKey;  
        });
        ...
        for (; count < projectLines.length; count++) {
            ...
            sublist.setSublistValue({ id: "custpage_start_date", line: count, value: toStr(p.startDate) });
            sublist.setSublistValue({ id: "custpage_end_date", line: count, value: toStr(p.endDate) });
            sublist.setSublistValue({ id: "custpage_closure_date", line: count, value: toStr(p.closureDate) });
            sublist.setSublistValue({ id: "custpage_project_name", line: count, value: toStr(p.projectName) });
    ...
    const getSortedProjects = ({ projectObject }) => {
        return Object.values(projectObject).sort((a, b) => {
            const aDate = Date.parse(a.creationDate) || -Infinity;
            const bDate = Date.parse(b.creationDate) || -Infinity;
    ...
    const renderTxnLinesByCodePlusJob = ({ jobId, projectCode, fromDate, uptoDate, nsTypes, pageLabel }) => {
        ...
        addCommonDetailColumns(sl, "Customer");
    ...
    const renderTxnLinesByCodePlusJobtotalpopb = ({ jobId, projectCode, fromDate, uptoDate, nsTypes, pageLabel }) => {
        ...
        const sl = form.addSublist({ id: "custpage_txn_list", type: serverWidget.SublistType.LIST, label: toStr(pageLabel) });
        addCommonDetailColumns(sl, "Vendor/Employee");
    ...
    const renderJvSalesByJob = ({ jobId, projectCode, fromDate, uptoDate }) => {
        ...
        results.forEach(r => {
            ...
            const amt = (toNum(r.getValue({ name: "amount" })));// Changed from absolute to signed in latest change test file
            total += amt;
    ...
    const renderJvPurchByJob = ({ jobId, projectCode, fromDate, uptoDate }) => {
        ...
        sl.addField({ id: "col_type", type: serverWidget.FieldType.TEXT, label: "Type" });
        sl.addField({ id: "col_date", type: serverWidget.FieldType.TEXT, label: "Date" });
        sl.addField({ id: "col_entity", type: serverWidget.FieldType.TEXT, label: "Vendor/Employee" });
        sl.addField({ id: "col_projcode", type: serverWidget.FieldType.TEXT, label: "Project Code" });
        sl.addField({ id: "col_account", type: serverWidget.FieldType.TEXT, label: "Account" });
        sl.addField({ id: "col_amount", type: serverWidget.FieldType.TEXT, label: "Amount" });
        sl.addField({ id: "col_createdby", type: serverWidget.FieldType.TEXT, label: "Created By" });
        ...
        results.forEach(r => {
            ...
            const href = resolveRecordInternal("journalentry", id);
            const link = href ? `<a href="${href}" target="_blank">${voucher}</a>` : voucher;
            const amt = (toNum(r.getValue({ name: "amount" })));// changed from abs to signed for latest change in test file
            total += amt;
    ...
    const renderTxnLinesByJob = ({ jobId, projectCode, fromDate, uptoDate, nsTypes, pageLabel, entityLabel = "Vendor" }) => {
        const title = `${toStr(pageLabel)} for Project Code ${toStr(projectCode)} [ ${toStr(fromDate, "-")} to ${toStr(uptoDate, "-")} ]`;
        const form = serverWidget.createForm({ title });
        ...
        sl.addField({ id: "col_type", type: serverWidget.FieldType.TEXT, label: "Type" });
        sl.addField({ id: "col_date", type: serverWidget.FieldType.TEXT, label: "Date" });
        sl.addField({ id: "col_entity", type: serverWidget.FieldType.TEXT, label: entityLabel });
        sl.addField({ id: "col_projcode", type: serverWidget.FieldType.TEXT, label: "Project Code" });
        sl.addField({ id: "col_account", type: serverWidget.FieldType.TEXT, label: "Account" });
        sl.addField({ id: "col_amount", type: serverWidget.FieldType.TEXT, label: "Amount" });
        sl.addField({ id: "col_createdby", type: serverWidget.FieldType.TEXT, label: "Created By" });
    ...
    const addCommonDetailColumns = (sl, entityLabel = "Customer/Vendor/Employee") => {
        sl.addField({ id: "col_voucher", type: serverWidget.FieldType.TEXT, label: "Voucher #" });
        sl.addField({ id: "col_type", type: serverWidget.FieldType.TEXT, label: "Type" });
        sl.addField({ id: "col_date", type: serverWidget.FieldType.TEXT, label: "Date" });
        sl.addField({ id: "col_entity", type: serverWidget.FieldType.TEXT, label: entityLabel });
    ...
    const renderNetExpenseByJob = ({ jobId, projectCode, fromDate, uptoDate }) => {
        ...
        addCommonDetailColumns(sl, "Vendor");
    ...
    const getProjectTransactions = (fromDate, uptoDate, selectedCustomers, selectedLocations, selectedDepartment) => {
        ...
        const s = search.create({
            ...
            columns: [
                ...
                search.createColumn({ name: "formuladate", summary: "GROUP", formula: "{job.custentity_project_code_date}", lable: "Project Creation Date", sort: search.Sort.DESC }),
                search.createColumn({ name: "formuladate", summary: "GROUP", formula: "{job.startdate}", lable: "Start Date" }),
                search.createColumn({ name: "formuladate", summary: "GROUP", formula: "{job.projectedenddate}", lable: "End Date" }),
                search.createColumn({ name: "formuladate", summary: "GROUP", formula: "{job.custentity_closure_date}", lable: "Closure date" }),
                ...
                search.createColumn({ name: "formulatext", summary: "GROUP", formula: "NVL({job.jobname},' ' )", label: "Project Name" }),
    ...
    const getDeligateSoSbCn = (fromDate, uptoDate, selectedCustomers, selectedLocations, selectedDepartment) => {
        ...
        log.debug('Deligates filter', filters);
        const s = search.create({
            type: "transaction",
            settings: [{ name: "consolidationtype", value: "ACCTTYPE" }],
            filters,
    ...
    const getDeligatesProject = (projectCode) => {
        const s = search.create({
            ...
            columns: [
                ...
                search.createColumn({ name: "formuladate", formula: "{startdate}", lable: "Start Date" }),
                search.createColumn({ name: "formuladate", formula: "{projectedenddate}", lable: "End Date" }),
                search.createColumn({ name: "formuladate", formula: "{custentity_closure_date}", lable: "Closure date" }),
                ...
                search.createColumn({ name: "formulatext", formula: "NVL({jobname},' ' )" }),
                search.createColumn({ name: "formulatext", formula: "NVL({custentity_billing_budget_history}, ' ')", label: "Billing Budget" }),
                search.createColumn({ name: "formulatext", formula: "NVL({custentity_cost_budget_history}, ' ')", label: "Cost Budget" }),
    ...
    const sortByCreationDateDesc = (dataObj) => {
        var dataArray = Object.values(dataObj);
        dataArray.sort(function (a, b) {
            var dateA = convertToDate(a.creationDate);
            var dateB = convertToDate(b.creationDate);
            return dateB - dateA;
        });
