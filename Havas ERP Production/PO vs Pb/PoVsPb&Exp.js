...
define(["N/ui/serverWidget", "N/search"], (ui, search) => {
    function onRequest(context) {
        ...
        const sublist = form.addSublist({
            ...
            label: "PO vs PB and Expense Report Data",
        });
        ...
        sublist.addField({ id: "vendor", label: "Vendor/Employee", type: ui.FieldType.TEXT });
        sublist.addField({ id: "po_amount", label: "PO Amount", type: ui.FieldType.CURRENCY }); // Get Total
        sublist.addField({ id: "po_to_be_booked", label: "PO to be Booked", type: ui.FieldType.CURRENCY });
        sublist.addField({ id: "purchase_bill_no", label: "Purchase Bill No./Expense Voucher No.", type: ui.FieldType.TEXT });
        ...
        sublist.addField({ id: "pb_amount", label: "PB Amount/Expense Amount", type: ui.FieldType.CURRENCY }); // Get Total
        ...
        if (context.request.method === "POST") {
            ...
            // Load SheetJS library into the page
            form.addField({
                id: 'custpage_xlsx_lib',
                type: ui.FieldType.INLINEHTML,
                label: 'hidden'
            }).defaultValue = '<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>';
            ...
            for (let projectName in resultsByProject) {
                resultsByProject[projectName].forEach(po => {
                    if (po.billIds.length === 0 && !_logValidation(fromDate) && !_logValidation(toDate)) {
                        ...
                        totalPOAmount += parseFloat(po.poAmount) || 0;
                        ...
                        po.billIds.forEach(billId => {
                            ...
                            if (billData) {
                                ...
                                totalPBAmount += parseFloat(billData.billAmount) || 0;
            ...
            if (selectedProjectId || (fromDate && toDate)) {
                ...
                if (expenseResults) {
                    expenseResults.forEach(result => {
                        ...
                        totalPBAmount += parseFloat(result.expenseAmount) || 0;
            ...
            sublist.setSublistValue({
                id: "po_amount",
                line: lineNum,
                value: totalPOAmount.toFixed(2)
            });
            sublist.setSublistValue({
                id: "pb_amount",
                line: lineNum,
                value: totalPBAmount.toFixed(2)
            });
