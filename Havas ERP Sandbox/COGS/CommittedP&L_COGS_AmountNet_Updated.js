  ...
  function getCommittedPL(projectId) {
    ...
    var profitCalculation =
      ...
      (parseFloat(cogsJSON["Purchase Order"]) +
        ...
        parseFloat(Math.abs(cogsJSON["Journal"])));

    if (
      _logValidation(parseFloat(profitCalculation))
    ) {
      var profitMargin =
        (profitCalculation / parseFloat(cogsJSON["Sales Order"])) * 100;

      cogsJSON["Profit Margin"] = Math.abs(profitMargin).toFixed(2);
    }

    log.debug("profitCalculation", profitCalculation);
    log.debug("profitMargin", profitMargin);
    log.debug(
      'formatIndianNumber(parseFloat(Math.abs(cogsJSON["Journal"]))) ',
      formatIndianNumber(parseFloat(Math.abs(cogsJSON["Journal"])))
    );
