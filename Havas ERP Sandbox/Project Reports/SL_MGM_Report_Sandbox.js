...
define(["N/ui/serverWidget", "N/search", "N/url"], (serverWidget, search, url) => {
    ...
    const THIS_SUITELET_INTERNAL = {
        scriptId: "customscript_mgm_report_sb",
    ...
    const generateMGMReport = (fromDate, uptoDate, selectedCustomers, selectedProjects, selectedLocations, selectedDepartment) => {
        ...
        // fDept.defaultValue = nameById("department", selectedDepartment);
        // fDept.updateDisplayType({ displayType: serverWidget.FieldDisplayType.INLINE });
    ...
    const searchDelegatedSOsByProjectCode = (projectCode, fromDate, uptoDate) => {
        ...
        const segTrimExpr =
            "CASE " +
            "WHEN {cseg_ags_project_co} IS NOT NULL THEN " +
            "  (CASE WHEN INSTR({cseg_ags_project_co}, ' ') > 0 " +
            "        THEN SUBSTR({cseg_ags_project_co}, 1, INSTR({cseg_ags_project_co}, ' ') - 1) " +
            "        ELSE {cseg_ags_project_co} END) " +
            "ELSE NULL END";
    ...
    const searchSalesOrdersByProjectCode = (projectCode, fromDate, uptoDate) => {
        ...
        //   "WHEN {cseg_ags_project_co} IS NOT NULL THEN " +
        //   "CASE WHEN INSTR({cseg_ags_project_co}, ' ') > 0 THEN SUBSTR({cseg_ags_project_co}, 1, INSTR({cseg_ags_project_co}, ' ') - 1) ELSE {cseg_ags_project_co} END " +
        //   "ELSE NULL END";
    ...
    const getDeligateSoSbCn = (fromDate, uptoDate, selectedCustomers, selectedLocations, selectedDepartment) => {
        ...
        const s = search.create({
            ...
            filters,
            columns: [
                search.createColumn({ name: "formulatext", summary: "GROUP", formula: "CASE WHEN {job.entityid} IS NOT NULL THEN CASE WHEN INSTR({job.entityid}, ' ') > 0 THEN SUBSTR({job.entityid}, 1, INSTR({job.entityid}, ' ') - 1) ELSE {job.entityid} END WHEN {cseg_ags_project_co} IS NOT NULL THEN CASE WHEN INSTR({cseg_ags_project_co}, ' ') > 0 THEN SUBSTR({cseg_ags_project_co}, 1, INSTR({cseg_ags_project_co}, ' ') - 1) ELSE {cseg_ags_project_co} END ELSE NULL END" }),
