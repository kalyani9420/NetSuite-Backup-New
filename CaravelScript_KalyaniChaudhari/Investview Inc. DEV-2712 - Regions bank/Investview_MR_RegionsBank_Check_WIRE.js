 ...
 */
/**
*Description: The Map Reduce script to generate Check files in WIRE XML format"
  Script Name: Investview_MR_RegionsBank_Check_WIRE.js
  Author: Akanksha Bhardwaj
  Company: Caravel
  Date: 13-03-2024 
  Script Modification Log:
  -- version--     -- Date --         -- Modified By --     --Requested By--      -- Description --
       1.0         13-03-2024         Akanksha Bhardwaj       Habib Abdullahi        Create a Map Reduce script to generate Check files in WIRE XML format
*/
define(["N/currentRecord", "N/search", "N/file", "N/format", "N/runtime", "N/record"], (
  ...
  const map = (mapContext) => {
    ...
    xmlValue += "\n\t\t<CdtrAgt>";
    xmlValue += "\n\t\t\t<FinInstnId>";
    if (_logValidation(entityBankBIC)) {
      xmlValue += "\n\t\t\t\t<BICOrBEI>" + entityBankBIC + "</BICOrBEI>";
    }
    else {
      log.debug('in continue of BIC', 'in continue of BIC');
    }
    xmlValue += "\n\t\t\t\t<ClrSysMmbId>";
    xmlValue += "\n\t\t\t\t\t<ClrSysId>";
    xmlValue += "\n\t\t\t\t\t\t<Cd>USABA</Cd>";
    ...
    xmlValue += "\n\t</Cdtr>";
    xmlValue += "\n\t<CdtrAcct>";
    if (_logValidation(entityBankIBAN)) {
      xmlValue += "\n\t\t<Id>";
      xmlValue += "\n\t\t\t<IBAN>" + entityBankIBAN + "</IBAN>";
      xmlValue += "\n\t\t</Id>";
    }
    else {
      log.debug('in continue of IBAN', 'in continue of IBAN');
      xmlValue += "\n\t\t<Id>";
      xmlValue += "\n\t\t\t<Othr>";
  ...
  const reduce = (reduceContext) => {
    ...
    var file_name = "InvestviewPayables_" + formatted_current_date + "_" + formatted_current_time + "_WIRE" + ".xml";
  ...
  function getPin() {
    //position - 2 , 6 , 8 , 11 , 13 , 16
    //pin - 240308
    var masked_pin = 4278245090310908;
